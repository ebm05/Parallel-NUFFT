...
void FGG_expansion(double *E, double x, int Mr, int Msp, double tau)
...
// Naive gridding -- no fast Gaussian gridding
{
...
void fgg3d_v2(complex<double> *ftau, complex<double> *f, double *x, double *y, double *z, int N, int Mr, int Msp, double tau)
// Fast Gaussian gridding in 3d (Greengaard & Lee 2004).
//
// Input:
//  ftau :  Complex array of size Mr*Mr*Mr that stores spreaded values on the
...
// EB 2024-01-28.
{
    ...
    for (int n = 0; n < N; n++)
    {
        ...
        // pre-multiply with E3 and store in E2
        for (int p=0; p<P; p++){
            E2x[p]*=E3[p];
            E2y[p]*=E3[p];
            E2z[p]*=E3[p];
        ...
        for (int ix = 0; ix < P; ix++)
        {
            complex<double> Vx = V0 * E2x[ix];
            int jx = positive_mod(m1 + ix - Msp + 1, Mr);
            for (int iy = 0; iy < P; iy++)
            {
                complex<double> Vy = Vx * E2y[iy];
                int jy = positive_mod(m2 + iy - Msp + 1, Mr);
                for (int iz = 0; iz < P; iz++)
                {
                    int jz = positive_mod(m3 + iz - Msp + 1, Mr);
...
# pragma omp critical
    ...
    free(E2x); free(E2y); free(E2z); free(ftau_local);
}
free(E3);
}
...
#pragma omp critical
        ...
        free(kx0); free(ky0); free(kz0); free(f0); free(ftau0);
        }
